...
class Pagination
{
	...
	public function create($page , $url , $totalObjectIds ,$div , $spinnerPath,$spinnerId,$limit=10){	
		...
		for($i = 1; $i <= $numofpages; $i++)
		{
			if($i > $page-10 and $i < $page+10)
			{
				if($page == $i){
					...
					$pagination .= '<a title="Page '.$i.'" onclick="simpleAjaxPaging(\''.$url.'\',\''.$div.'\',\''.$spinnerId.'\');"">'.$i.'</a>&nbsp;';
				}
			}
		}			
		...
		{
			$pagenext = ($page + 1);
			$pagination .= '&nbsp;<a title="Next"  '.$onClick1.$PageLink.'page='.$pagenext.$qParams.$onClick2.' href="#'.$reFresh.'">Next</a>';
		}		
		else{
