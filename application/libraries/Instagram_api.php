...
*/
    function getUserFeed($max = null,$count=null) {
  
     $user_feed_request_url = sprintf($this->api_urls['user_feed'], $this->access_token, $max,$count);
	 $url = explode("&",$user_feed_request_url);
	if($max ==null){
		$url1=$url[0]."&count=".$count;
	}else{
		$url1=$url[0]."&max_id=".$max."&count=".$count;
	}
	
     return $this->__apiCall($url1);
...
*/
	...
	public function objectToArray($d){
		...
		if (is_array($d)) {
			...
			* for recursive call
			*/
			return $d;//array_map(__FUNCTION__, $d);
		}
		else {
			// Return array
			return $d;
